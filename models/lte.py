...
class LTE(nn.Module):
    ...
    def __init__(self, hidden_dim=256, wKAN:bool=False):
        super().__init__()        
        self.encoder = SwinIR()
        ...
        if wKAN:
            self.imnet = KAN(in_dim=hidden_dim, out_dim=1)
        else:
            self.imnet = MLP(in_dim=hidden_dim, out_dim=1, hidden_list=[256, 256, 256])
    ...
    def query_rgb(self, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # prepare cell
                rel_cell = 2 / (cell.unsqueeze(1).repeat(1, coord.shape[1], 1))
                # rel_cell = cell.clone()
