...
class UNet(nn.Module):
    ...
    def forward(self, x_noisy, x_lr, scaler, time):
        x = torch.cat([x_lr, x_noisy], dim=1)
        return self.forward_unet(x, x_lr, scaler, time) #if self.predict_noise else x_noisy + self.forward_unet(x, x_lr, scaler, time)
...
class UNet_wout_Scale(nn.Module):
    ...
    def forward(self, x_noisy, x_lr, time):
        x = torch.cat([x_lr, x_noisy], dim=1)
        return self.forward_unet(x, x_lr, time) #if self.predict_noise else x_noisy + self.forward_unet(x, x_lr, scaler, time)
