...
class UNet_wout_Scale(nn.Module):
    ...
    def forward(self, x_noisy, x_lr, t):
        # x = torch.cat([x_lr, x_noisy], dim=1)
        x = self.forward_unet(torch.cat([x_lr, x_noisy], dim=1), x_lr, t) #if self.predict_noise else x_noisy + self.forward_unet(x, x_lr, scaler, time)
        t = t - self.eps
